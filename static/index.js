...
function gameTime() {
    _gameTimeNum--;
    if (_gameTimeNum <= 0) {
        GameTimeLayer.innerHTML = '&nbsp;&nbsp;&nbsp;&nbsp;啦啦啦，时间到！';
        gameOver();
...
function shareText(score) {
    ...
    SubmitResults();
    if (score <= 59) return '「嗯，新型炸弹研究完毕！趁琴团长发现之前，开溜！」';
    if (score <= 79) return '「又进步了一点！不过琴团长总是说：「进步是分内之事。」」';
    if (score <= 109) return '「谢谢你总是陪我一起收集研究用的材料！如果你想学习炸弹的制作方法，我也可以来教你哦。」';
    if (score <= 119) return '谢谢你总是帮可莉解决麻烦！就用这串「可莉烤鱼」来报答你吧！——呃，其实鱼是直接在湖里就被炸弹烤熟了，不过这件事就和平时一样，假装不知道好了。';
    return '很久以前，第一听到爆炸声的时候，我没有害怕…后来我才知道，大部分人不是这样的…不过，你也不害怕我的炸弹呢！嘿嘿，太好了，可莉果然交到了很好的朋友！';
}
...
console.log("不修改，好嘛？乱传又有什么用呢？(ˉ▽ˉ；)...")///不行了啦，我也是受人之托，我就是个臭玩电脑的，我又有什么错呢(ˉ▽ˉ；)？
