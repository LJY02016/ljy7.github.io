...
function shareText(score) {
    ...
    SubmitResults();
    if (score <= 49) return '「嗯，新型炸弹研究完毕！趁琴团长发现之前，开溜！」';
    if (score <= 99) return '「又进步了一点！不过琴团长总是说：「进步是分内之事。」」';
    if (score <= 149) return '「谢谢你总是陪我一起收集研究用的材料！如果你想学习炸弹的制作方法，我也可以来教你哦。」';
    if (score <= 199) return '谢谢你总是帮可莉解决麻烦！就用这串「可莉烤鱼」来报答你吧！——呃，其实鱼是直接在湖里就被炸弹烤熟了，不过这件事就和平时一样，假装不知道好了。';
